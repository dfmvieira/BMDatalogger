const char Progbar2_START[][8] PROGMEM = {
  ...
  /*{ B01111, B11000, B10000, B10000, B10000, B10011, B11000, B01111 },
  { B01111, B11000, B10000, B10000, B10111, B10011, B11000, B01111 },
  { B01111, B11000, B10000, B10111, B10111, B10011, B11000, B01111 },*/
...
const char Progbar2_MIDDLE[][8] PROGMEM = {
  ...
  /*{ B11111, B00000, B00000, B00000, B00000, B11000, B00000, B11111 },
  { B11111, B00000, B00000, B00000, B11000, B11000, B00000, B11111 },
  { B11111, B00000, B00000, B11000, B11000, B11000, B00000, B11111 },*/
  { B11111, B00000, B11000, B11000, B11000, B11000, B00000, B11111 },
  /*{ B11111, B00000, B11000, B11000, B11000, B11011, B00000, B11111 },
  { B11111, B00000, B11000, B11000, B11011, B11011, B00000, B11111 },
  { B11111, B00000, B11000, B11011, B11011, B11011, B00000, B11111 },*/
...
const char Progbar2_END[][8] PROGMEM = {
  ...
  /*{ B11110, B00011, B00001, B00001, B00001, B11001, B00011, B11110 },
  { B11110, B00011, B00001, B00001, B11101, B11001, B00011, B11110 },
  { B11110, B00011, B00001, B11101, B11101, B11001, B00011, B11110 },*/
...
void CharacterInit(byte bank) {
  if (InitMode != bank) {
    ...
    switch (bank) {
      ...
      /*case 2:
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_START[4][i2] ); lcd.createChar (0, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[0][i2] ); lcd.createChar (1, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[8][i2] ); lcd.createChar (2, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_END[0][i2] ); lcd.createChar (3, ThisArray);
        break;
    
      case 3:
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_START[0][i2] ); lcd.createChar (4, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_START[1][i2] ); lcd.createChar (5, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_START[2][i2] ); lcd.createChar (6, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_START[3][i2] ); lcd.createChar (7, ThisArray);
        break;
  
      case 4:
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[1][i2] ); lcd.createChar (4, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[2][i2] ); lcd.createChar (5, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[3][i2] ); lcd.createChar (6, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[4][i2] ); lcd.createChar (7, ThisArray);
        break;
  
      case 5:
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[4][i2] ); lcd.createChar (4, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[5][i2] ); lcd.createChar (5, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[6][i2] ); lcd.createChar (6, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_MIDDLE[7][i2] ); lcd.createChar (7, ThisArray);
        break;
  
      case 6:
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_END[1][i2] ); lcd.createChar (4, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_END[2][i2] ); lcd.createChar (5, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_END[3][i2] ); lcd.createChar (6, ThisArray);
        for (int i2=0; i2<8; i2++) ThisArray[i2]= (uint8_t) pgm_read_byte( &Progbar2_END[4][i2] ); lcd.createChar (7, ThisArray);
        break;*/
    }
